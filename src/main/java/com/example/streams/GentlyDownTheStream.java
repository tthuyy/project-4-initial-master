...
public class GentlyDownTheStream {
    ...
    public List<String> reverseSortedVeggiesInUpperCase() {
        return veggies.stream().sorted(Comparator.reverseOrder()).map(x -> x.toUpperCase())
                .collect(Collectors.toList());
    }
    ...
    public List<Integer> topTenUnique() {
        return integerValues.stream().distinct().sorted(Comparator.reverseOrder()).limit(10)
                .collect(Collectors.toList());
    }
    ...
    public List<Integer> topTenUniqueOdd() {
        return integerValues.stream().filter(x -> x % 2 == 1)
